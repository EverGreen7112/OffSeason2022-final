...
public class Controls {
  ...
  private static JoystickButton m_collectorCollect, m_collectorUncollect, m_collectorOpen, m_collectorClose,
      m_climberDown,
      m_climberUp, m_storageUp, m_storageDown, m_load, m_turnToShoot, m_driveStright,
  ...
  public static void driveStright() {
    ...
    // speed = calcTrajectory.calcSpeed(distance,
    // Constants.PhysicalConsts.SHOOT_HEIGHT,
  ...
  public static void shoot() {
    ...
    // Constants.UsableMotors.FLY_WHEEL.set(TalonSRXControlMode.Position, Constants.UsableMotors.FLY_WHEEL.getSelectedSensorPosition() + Constants.Values.TICKS_PER_REVOLUTIONS * 3);
  }

  public static void shootAuto() {
    float distance = m_hubVision.getZ();
    // double speed = calcTrajectory.calcSpeed(distance,
    // Constants.PhysicalConsts.SHOOT_HEIGHT,
    // Constants.PhysicalConsts.SHOOT_ANGLE);
    // m_shootPidMotor.set(speed * Constants.Speeds.SHOOT);
    // m_shootPidMotor.runMotor();
  ...
  public static void movePeriodic() {
    ...
    // Climb
    Constants.UsableMotors.CLIMBER_MOTOR
        .set(((m_climberUp.get() ? 1 : 0) + (m_climberDown.get() ? -1 : 0)) * Constants.Speeds.climberMotor);
