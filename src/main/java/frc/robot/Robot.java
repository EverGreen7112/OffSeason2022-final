...
public class Robot extends TimedRobot {
  ...
  public void testPeriodic() {
    ...
    double m_flyWheelAngle = ticksToAngle(Constants.UsableMotors.FLY_WHEEL.getSelectedSensorPosition()) % 360;
    if(Math.abs(m_flyWheelAngle - m_JoystickAngle) > 180){
      m_JoystickAngle += 360;
    } 
    double m_flyWheelTarget = angleToTicks(m_JoystickAngle);
    m_JoystickAngle = mod(m_JoystickAngle, 360);
